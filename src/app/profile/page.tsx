...
const Profile = () => {
    ...
    return (
        <div className="h-screen max:h-screen-auto flex justify-center items-center bg-bg-color">
            <div className="w-1/2 rounded-md bg-custom-prof flex justify-between flex-col font-mono">
                <div className="h-28 w-full justify-center flex items-center">
                    <span className="text-3xl text-white font-mono font-semibold p-3 rounded-lg">Welcome Back {userInfo?.displayName}</span>
                </div>
                <div className="flex items-start justify-evenly flex-col w-1/2 self-center ">
                    <div className="text-white font-bold text-lg">Email: {userInfo?.email}</div>
                    <div className="text-white font-bold text-lg">Name: {userInfo?.displayName}</div>
                </div>

                <div className="flex w-full items-center justify-around my-4">
                    <span className="cursor-pointer py-1 px-2 bg-custom-profile rounded-md text-black" onClick={()=>setVisibility("profile")}>Update Profile</span>
                    <span className="cursor-pointer py-1 px-2 bg-custom-profile rounded-md text-black" onClick={()=>setVisibility("password")}>Change Password</span>
                </div>
